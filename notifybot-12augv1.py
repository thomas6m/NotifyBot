...
========
    ...
    parser.add_argument("--test-inventory", 
                       metavar="PATH",
                       help="Custom path to test inventory CSV file (requires --test flag to be effective)")
    ...
    # FIXED: Set up logging FIRST before any other operations
    setup_logging()
    
    # FIXED: Add validation for --test-inventory without --test AFTER logging is set up
    if args.test_inventory and not args.test:
        ...
        args.test_inventory = None  # Clear it to avoid confusion
    
    # Now it's safe to call functions that might use log_and_print
    if args.test:
        if args.test_inventory:
            log_and_print("mode", f"TEST MODE: Using custom test inventory: {args.test_inventory}")
            # Log the successful validation that we couldn't log earlier
