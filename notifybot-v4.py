...
# ADD THIS DISCLAIMER SECTION IMMEDIATELY AFTER RECIPIENT CALCULATION
            if filter_file_path.is_file() and INVENTORY_PATH.is_file():
                # ANSI escape codes
                BOLD = '\033[1m'
                RED = '\033[91m'
                YELLOW = '\033[93m'
                ...
                if to_file_path.is_file():
                    ...
                    print(f"{BOLD}{YELLOW}⚠️  DISCLAIMER: Dry-run will create to.txt from filter results for review{RESET}")
                    print(f"{BOLD}{BLUE}💡 This allows you to verify filtered recipients before live sending{RESET}")
                    print(f"{BOLD}📝 Will create to.txt with {original_recipients_count} filtered recipients{RESET}")
    
                print()
                print(f"{BOLD}{RED}{'=' * 80}{RESET}")
                print()  # Print blank line for better readability
    
    # Also log to file normally
                if to_file_path.is_file():
                    ...
                    log_and_print("info", "⚠️  DISCLAIMER: Dry-run will create to.txt from filter results for review")
                    log_and_print("info", "💡 This allows you to verify filtered recipients before live sending")
                    log_and_print("info", f"📝 Will create to.txt with {original_recipients_count} filtered recipients")
