...
def create_email_message(recipients: List[str], subject: str, body_html: str, 
    ...
    return msg
def matches_promql_filter_conditions(row: Dict, filters: List[str]) -> bool:
    """
    Check if a row matches the PromQL-style filter conditions.
    Each line in filters represents an OR condition.
    Within each line, comma-separated conditions are AND conditions.
    
    Supports PromQL operators:
    - = : Select labels that are exactly equal to the provided string
    - != : Select labels that are not equal to the provided string  
    - =~ : Select labels that regex-match the provided string
    - !~ : Select labels that do not regex-match the provided string
    
    Example: environment=~"staging|testing|development",method!="GET"
    """
    if not filters:
        return True  # No filters means include all
    
    def matches_promql_condition(text: str, operator: str, pattern: str) -> bool:
        """Check if text matches pattern using PromQL operator."""
        text = str(text)
        ...
        if operator == '=':
            ...
            try:
                # PromQL regex matches are fully anchored
                ...
                log_and_print("warning", f"Invalid regex pattern '{pattern}': {e}")
                return False
        elif operator == '!~':
            ...
            try:
                ...
                log_and_print("warning", f"Invalid regex pattern '{pattern}': {e}")
                return True  # If regex is invalid, assume no match (so !~ returns True)
        else:
            log_and_print("warning", f"Unknown PromQL operator: {operator}")
            return False
    ...
    for filter_line in filters:
        ...
        for condition in and_conditions:
            ...
            # Parse PromQL condition: key operator "value"
            # Supported operators: =, !=, =~, !~
            promql_pattern = r'^([^=!~]+)(=~|!~|!=|=)\s*"?([^"]*)"?\s*$'
            match = re.match(promql_pattern, condition)
            ...
            if match:
                ...
                # Use PromQL matching logic
                if not matches_promql_condition(row[key], operator, value):
                    line_matches = False
                    break  # This AND condition failed
            else:
                # Fallback: treat as simple string search in all values (for backward compatibility)
                log_and_print("warning", f"Invalid PromQL syntax, using fallback search: {condition}")
                condition_matched = False
                for row_value in row.values():
                    if condition.lower() in str(row_value).lower():
...
def apply_promql_filter_logic(filters: List[str], inventory_path: Path) -> List[str]:
    """Apply the PromQL-style filter logic using 'filter.txt' and 'inventory.csv'."""
    filtered_recipients = []
    ...
    try:
        with open(inventory_path, mode="r", newline="", encoding="utf-8") as file:
            ...
            for row in reader:
                if matches_promql_filter_conditions(row, filters):
                    if 'email' in row:
        ...
        log_and_print("info", f"PromQL filter applied: {len(filtered_recipients)} unique recipients selected from inventory")
        
    except Exception as exc:
        log_and_print("error", f"Error applying PromQL filter logic: {exc}")
    
    return filtered_recipients

def extract_field_values_from_promql_filter(filter_line: str, field_names: List[str]) -> Dict[str, str]:
    """
    Extract field values from a PromQL filter line for template substitution.
    
    Args:
        filter_line: Single filter line like 'department="sales",region=~"north|south"'
    ...
    # Parse PromQL filter conditions
    and_conditions = [condition.strip() for condition in filter_line.split(',')]
    ...
    for condition in and_conditions:
        ...
        if match:
            ...
            # For template substitution, we only use exact matches (=)
            # For regex matches (=~), we'll use the pattern as-is
            if key in field_names:
                if operator == '=':
                    field_values[key] = value
                elif operator == '=~':
                    # For regex patterns, we could extract the first alternative
                    # or use the pattern as-is. Let's use the pattern for now.
                    field_values[key] = value
                # For != and !~ operators, we can't determine a positive value
                # so we'll leave them empty
...
def get_recipients_for_single_mode(base_folder: Path, dry_run: bool) -> Tuple[List[str], List[str], List[str], int, int, int]:
    """
    Get recipients for single mode operation (updated to use PromQL logic).
    ...
    if dry_run:
        ...
        if to_file_path.is_file():
            ...
            # Use PromQL filter logic for single mode too
            filtered_recipients = apply_promql_filter_logic(filters, INVENTORY_PATH)
        ...
        # Priority 2: Use PromQL filter logic if to.txt doesn't exist
        elif filter_file_path.is_file() and INVENTORY_PATH.is_file():
            filters = read_file(filter_file_path).splitlines()
            # Use PromQL filter logic
            recipients = apply_promql_filter_logic(filters, INVENTORY_PATH)
            log_and_print("info", f"Loaded {len(recipients)} recipients from PromQL filter logic")
            ...
            if recipients:
                ...
                log_and_print("file", f"Created to.txt with {len(recipients)} merged recipients (PromQL filter + additional)")
...
def get_recipients_for_multi_mode(base_folder: Path, dry_run: bool) -> Tuple[List[Dict], List[str], List[str], int, int, int]:
    """
    Get recipients for multi mode operation with additional_to.txt support.
    ...
    # Read additional_to.txt recipients (NEW - support for multi mode)
    ...
    if additional_to_file.is_file():
        ...
        if additional_recipients:
            log_and_print("info", f"Loaded {len(additional_recipients)} additional recipients from additional_to.txt (will be added to each filtered email)")
    ...
    for i, filter_line in enumerate(filters, 1):
        log_and_print("processing", f"Processing PromQL filter {i}/{len(filters)}: {filter_line}")
        
        # Get recipients for this specific filter using PromQL logic
        filter_recipients = apply_promql_filter_logic([filter_line], INVENTORY_PATH)
        
        # Add additional_to.txt recipients to each filter's results
        if additional_recipients:
            original_filter_count = len(filter_recipients)
            filter_recipients = merge_recipients(filter_recipients, additional_recipients)
            added_count = len(filter_recipients) - original_filter_count
            if added_count > 0:
                log_and_print("info", f"Filter {i}: Added {added_count} recipients from additional_to.txt")
        ...
        # Extract field values for substitution using PromQL parser
        field_values = {}
        if field_names:
            field_values = extract_field_values_from_promql_filter(filter_line, field_names)
    ...
    if additional_recipients:
        log_and_print("info", f"Additional recipients from additional_to.txt will be included in ALL {len(email_configs)} emails")
