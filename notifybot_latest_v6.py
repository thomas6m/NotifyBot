...
def send_via_sendmail(recipients: List[str], subject: str, body_html: str, 
    ...
    try:
        ...
        sendmail_cmd = [sendmail_path, '-t', '-f', from_address] + all_recipients_for_delivery
...
def send_email_batch(recipients: List[str], subject: str, body_html: str, 
    ...
    if dry_run:
        ...
        log_and_print("processing", f"Original campaign would target: {total_original} recipients (TO: {original_recipients_count}, CC: {original_cc_count}, BCC: {original_bcc_count})")
    else:
        total_all_recipients = total_recipients + len(cc_recipients) + len(bcc_recipients)
        log_and_print("processing", f"Starting batch email send: {total_all_recipients} total recipients")
        log_and_print("processing", f"TO: {total_recipients}, CC: {len(cc_recipients)}, BCC: {len(bcc_recipients)}")
