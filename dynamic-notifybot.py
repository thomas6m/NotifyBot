...
def validate_fields_with_priority(base_folder: Path, mode: str = "single") -> Tuple[bool, List[str]]:
    ...
    # NEW: RULE 2 & 3: Validate table-columns.txt - ONLY when it exists
    ...
    if table_columns_file.is_file():
        try:
            ...
            if table_column_names:
                ...
                if has_local_field_inventory:
                    # RULE 2: Use local field inventory for table-columns.txt validation
                    inventory_to_use = local_available_fields
                    inventory_name = "local field-inventory.csv"
                    log_and_print("info", f"Validating table-columns.txt against local field-inventory.csv (priority)")
                else:
                    # RULE 3: Fallback to global inventory for table-columns.txt validation
                    inventory_to_use = global_available_fields
                    inventory_name = "global inventory.csv"
                    log_and_print("info", f"Validating table-columns.txt against global inventory.csv (fallback)")
                
                invalid_table_fields = []
                
                for line_num, field_name in enumerate(table_column_names, 1):
                    # Strip whitespace from field name from table-columns.txt as well
                    field_name = field_name.strip()
                ...
                if invalid_table_fields:
                    log_and_print("error", f"Invalid fields in table-columns.txt ({inventory_name}): {', '.join(invalid_table_fields)}")
                else:
                    log_and_print("info", f"All {len(table_column_names)} field names in table-columns.txt validated successfully against {inventory_name}")
            else:
    ...
    # No need to log anything if table-columns.txt doesn't exist - it's optional
