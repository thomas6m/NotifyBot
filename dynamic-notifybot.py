...
def extract_field_values_from_matched_rows(filter_line: str, field_names: List[str], inventory_path: Path, base_folder: Path) -> Dict[str, str]:
    ...
    ENHANCED: Now supports table row generation for microservices data.
...
"""
            ...
            log_and_print("info", f"Generated HTML table with {len(matched_rows_data)} rows for microservices data")
        ...
        if matched_rows_count > 0:
            ...
            if 'microservice_table_rows' in field_values:
                log_and_print("info", f"  - Generated microservice table with {matched_rows_count} rows")
