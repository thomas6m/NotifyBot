...
def validate_fields_with_priority(base_folder: Path, mode: str = "single") -> Tuple[bool, List[str]]:
    ...
    DYNAMIC_FIELDS = {
        ...
        'microservice_table_header',    # Generated HTML table header for microservices data  
        'microservice_complete_table',  # Complete HTML table (header + rows) for microservices data
            ...
            try:
                ...
                if field_names:
                    ...
                    for line_num, field_name in enumerate(field_names, 1):
                        ...
                        # Check if it's a dynamic field first
...
def check_attachment_size_limit(base_folder: Path, max_size_mb: int = 15) -> None:
    """
    Check if total size of all files in attachment folder exceeds the limit.
    
    Args:
        base_folder: Base folder path
        max_size_mb: Maximum allowed total size in MB (default: 15MB)
    
    Raises:
        MissingRequiredFilesError: If attachment size exceeds limit
    """
...
def extract_field_values_from_matched_rows(filter_line: str, field_names: List[str], inventory_path: Path, base_folder: Path) -> Dict[str, str]:
    ...
    ENHANCED: Now supports dynamic table row generation for microservices data with configurable columns.
    ENHANCED: Automatically detects table structure from available fields and generates appropriate HTML.
    
    Args:
        filter_line: Single filter line like "applicationid=1234"
    ...
    Returns:
        ...
        Also includes 'microservice_table_rows' for HTML table generation
    """
    field_values = {}
    ...
    try:
        ...
        # ENHANCED: Dynamic HTML table generation with configurable columns
        if matched_rows_data:
            # Define common microservice table configurations
            table_configs = {
                'microservice_basic': ['sector', 'region', 'clustername', 'namespace', 'microservicename'],
                'microservice_detailed': ['sector', 'region', 'clustername', 'namespace', 'microservicename', 'replicacount'],
                'microservice_full': ['applicationid', 'sector', 'region', 'clustername', 'namespace', 'microservicename', 'replicacount', 'status'],
                'infrastructure': ['region', 'clustername', 'namespace', 'status'],
                'deployment': ['microservicename', 'namespace', 'replicacount', 'status'],
            }
            
            # Auto-detect best table configuration based on available fields
            available_row_fields = set(matched_rows_data[0].keys())
            best_config = None
            best_match_count = 0
            
            for config_name, config_fields in table_configs.items():
                match_count = len([f for f in config_fields if f in available_row_fields])
                if match_count > best_match_count:
                    best_match_count = match_count
                    best_config = config_fields
            
            # If no predefined config matches well, create a dynamic one
            if best_match_count < 3:  # Less than 3 matching fields
                # Create dynamic config with common fields in logical order
                priority_fields = ['applicationid', 'sector', 'region', 'clustername', 'namespace', 
                                 'microservicename', 'replicacount', 'status', 'environment', 'version']
                best_config = [f for f in priority_fields if f in available_row_fields]
                
                # Add any remaining fields not in priority list
                remaining_fields = [f for f in available_row_fields if f not in best_config]
                best_config.extend(sorted(remaining_fields))
            
            # Generate table header HTML
            table_header = "        <tr>\n"
            for field in best_config:
                if field in available_row_fields:
                    # Create user-friendly column headers
                    display_name = field.replace('_', ' ').title()
                    if field == 'applicationid':
                        display_name = 'App ID'
                    elif field == 'microservicename':
                        display_name = 'Microservice'
                    elif field == 'replicacount':
                        display_name = 'Replicas'
                    elif field == 'clustername':
                        display_name = 'Cluster'
                    
                    table_header += f'            <th style="padding: 8px; border: 1px solid #ddd; background-color: #f5f5f5; font-weight: bold;">{display_name}</th>\n'
            table_header += "        </tr>\n"
            ...
            for row in matched_rows_data:
                ...
                for field in best_config:
                    if field in available_row_fields:
                        ...
                        if len(str(cell_value)) > 30:
                            cell_value = str(cell_value)[:27] + '...'
                        
                        table_rows_html += f'            <td style="padding: 8px; border: 1px solid #ddd;">{cell_value}</td>\n'
                table_rows_html += "        </tr>\n"
            
            # Store both header and rows for flexible template usage
            field_values['microservice_table_header'] = table_header.rstrip('\n')
            field_values['microservice_table_rows'] = table_rows_html.rstrip('\n')
            
            # Also create a complete table HTML for easy usage
            complete_table = f"""<table style="border-collapse: collapse; width: 100%; margin: 10px 0;">
{table_header}{table_rows_html}</table>"""
            field_values['microservice_complete_table'] = complete_table.strip()
            
            log_and_print("info", f"Generated dynamic HTML table with {len(matched_rows_data)} rows and {len(best_config)} columns")
            log_and_print("info", f"Table columns: {', '.join(best_config)}")
        ...
        if matched_rows_count > 0:
            ...
            # Log table generation info
            if 'microservice_table_rows' in field_values:
                log_and_print("info", f"  - Generated dynamic microservice table with {matched_rows_count} rows")
