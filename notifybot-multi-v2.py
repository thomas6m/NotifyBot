...
Supports both Single Mode (bulk emails) and Multi Mode (personalized emails).
...
Usage:
    ...
    python notifybot.py --base-folder emails --mode multi --dry-run
...
CLI Options:
    --base-folder         Base directory containing email input files [REQUIRED]. 
                          ...
                          Recipient source (at least one required for real email mode):
                            - to.txt                    List of recipient emails (Single Mode)
                          ...
                          Multi Mode specific:
                            - mode.txt                  Contains "multi" to enable Multi Mode
                            - field.txt                 Field names for placeholder substitution
...
def determine_mode(base_folder: Path, cli_mode: str = None) -> str:
    """Determine operation mode in priority order: CLI > mode.txt > default (single)"""
    
    # Priority 1: CLI override
    if cli_mode:
        if cli_mode.lower() not in ['single', 'multi']:
            raise ValueError(f"Invalid mode '{cli_mode}'. Must be 'single' or 'multi'.")
        log_and_print("info", f"Mode set via CLI: {cli_mode.lower()}")
    ...
    if mode_file.is_file():
        try:
            ...
            if mode_content in ['single', 'multi']:
                log_and_print("info", f"Mode set via mode.txt: {mode_content}")
                return mode_content
            else:
    ...
    # Priority 3: Default to single mode
    log_and_print("info", "No mode specified, defaulting to 'single' mode")
    return 'single'
...
def read_field_names(path: Path) -> List[str]:
    """Read field names from field.txt file."""
    if not path.is_file():
        log_and_print("info", f"{path.name} not found, using default field extraction")
        return []
    ...
    try:
        ...
        for line in path.read_text(encoding="utf-8").splitlines():
            ...
            if field and not field.startswith('#'):
                fields.append(field)
        log_and_print("info", f"Loaded {len(fields)} field names from {path.name}")
        return fields
    except Exception as exc:
        log_and_print("error", f"Error reading field names from {path}: {exc}")
        return []
...
def check_required_files(base: Path, required: List[str], dry_run: bool = True, mode: str = 'single') -> None:
    ...
    if not dry_run:
        if mode == 'single':
            ...
            # Multi mode requires filter.txt and inventory.csv
            if not (base / "filter.txt").is_file():
                raise MissingRequiredFilesError("Multi mode requires 'filter.txt'")
            if not INVENTORY_PATH.is_file():
                raise MissingRequiredFilesError("Multi mode requires 'inventory.csv' in /notifybot/inventory/")
...
def extract_field_values_from_filter(filter_condition: str, field_names: List[str]) -> Dict[str, str]:
    """Extract field values from a filter condition string."""
    field_values = {}
    
    # Parse the filter condition (comma-separated key=value pairs)
    conditions = [condition.strip() for condition in filter_condition.split(',')]
    ...
    for condition in conditions:
        if '=' in condition:
            ...
            # If field_names is specified, only extract specified fields
...
def apply_single_filter_logic(filter_condition: str, inventory_path: Path) -> List[str]:
    """Apply a single filter condition and return matching recipients."""
    return apply_filter_logic([filter_condition], inventory_path)
...
def send_multi_mode_emails(filter_lines: List[str], subject_template: str, body_template: str,
                          from_address: str, field_names: List[str], dry_run: bool = False,
                          attachment_folder: Path = None, base_folder: Path = None,
                          cc_recipients: List[str] = None, bcc_recipients: List[str] = None,
                          approver_emails: List[str] = None, delay: float = 5.0) -> None:
    """Send individual personalized emails for each filter condition (Multi Mode)."""
    
    cc_recipients = cc_recipients or []
    bcc_recipients = bcc_recipients or []
    approver_emails = approver_emails or []
    
    total_emails = len(filter_lines)
    successful_emails = 0
    failed_emails = 0
    total_original_recipients = 0
    
    log_and_print("multi", f"MULTI MODE: Processing {total_emails} individual email(s)")
    
    for i, filter_condition in enumerate(filter_lines, 1):
        filter_condition = filter_condition.strip()
        
        # Skip empty lines and comments
        if not filter_condition or filter_condition.startswith('#'):
            log_and_print("info", f"Skipping empty/comment line {i}")
            continue
        
        log_and_print("multi", f"Processing email {i}/{total_emails}: {filter_condition}")
        
        # Extract field values for personalization
        field_values = extract_field_values_from_filter(filter_condition, field_names)
        ...
        if dry_run:
            # In dry-run mode, send to approvers instead
            email_recipients = approver_emails
            # Still calculate original recipients for informational purposes
            original_recipients = apply_single_filter_logic(filter_condition, INVENTORY_PATH)
            original_count = len(original_recipients)
            total_original_recipients += original_count
        else:
            # In live mode, send to actual filtered recipients
            email_recipients = apply_single_filter_logic(filter_condition, INVENTORY_PATH)
            original_count = len(email_recipients)
            total_original_recipients += original_count
        
        if not email_recipients:
            log_and_print("warning", f"No recipients found for filter: {filter_condition}")
            failed_emails += 1
            continue
        
        # Log personalization info
        if field_values:
            field_info = ", ".join([f"{k}={v}" for k, v in field_values.items()])
            log_and_print("multi", f"Personalized fields: {field_info}")
        ...
        try:
            if send_via_sendmail(
                ...
                attachment_folder, 
                dry_run, 
                original_count,  # Original recipient count for this specific email
                base_folder,
                cc_recipients,   # CC recipients get every email
                bcc_recipients,  # BCC recipients get every email
                len(cc_recipients),
                len(bcc_recipients)
            ):
                successful_emails += 1
                if dry_run:
                    log_and_print("success", f"DRAFT email {i} sent to approvers (would target {original_count} recipients)")
                else:
                    total_sent = len(email_recipients) + len(cc_recipients) + len(bcc_recipients)
                    log_and_print("success", f"Email {i} sent successfully to {total_sent} recipients")
            else:
                failed_emails += 1
                log_and_print("error", f"Email {i} failed to send")
        except Exception as exc:
            failed_emails += 1
            log_and_print("error", f"Email {i} failed with exception: {exc}")
        
        # Add delay between emails (except for the last one)
        if i < total_emails and not dry_run and delay > 0:
            log_and_print("info", f"Waiting {delay} seconds before next email...")
            time.sleep(delay)
    
    # Multi-mode summary
    log_and_print("multi", f"MULTI MODE complete: {successful_emails} successful, {failed_emails} failed")
    
    if dry_run:
        log_and_print("multi", f"DRAFT mode: {successful_emails} draft emails sent to approvers")
        log_and_print("multi", f"Original campaign would send {successful_emails} individual emails to {total_original_recipients} total recipients")
        if cc_recipients or bcc_recipients:
            cc_bcc_total = (len(cc_recipients) + len(bcc_recipients)) * successful_emails
            log_and_print("multi", f"CC/BCC would receive {cc_bcc_total} total emails ({successful_emails} emails each)")
    else:
        total_delivered = total_original_recipients + (len(cc_recipients) * successful_emails) + (len(bcc_recipients) * successful_emails)
        log_and_print("multi", f"Total emails delivered: {total_delivered}")
        if cc_recipients or bcc_recipients:
            log_and_print("multi", f"CC/BCC recipients received {successful_emails} individual emails each")
...
def main():
    """Updated main function with Single and Multi Mode support"""
    parser = argparse.ArgumentParser(description="Send batch emails with Single and Multi Mode support.")
    ...
    parser.add_argument("--delay", type=float, default=5.0, help="Delay in seconds between batches/emails (default: 5.0).")
    ...
    try:
        ...
        operation_mode = determine_mode(base_folder, args.mode)
        
        # Check required files based on mode
        required_files = ["subject.txt", "body.html", "from.txt", "approver.txt"]
        check_required_files(base_folder, required_files, args.dry_run, operation_mode)
        
        # Read email content
        subject_template = read_file(base_folder / "subject.txt")
        body_template = read_file(base_folder / "body.html")
        ...
        if operation_mode == 'multi':
            ...
            log_and_print("multi", "=== MULTI MODE PROCESSING ===")
            
            # Read filter conditions and field names
            filter_file_path = base_folder / "filter.txt"
            field_file_path = base_folder / "field.txt"
            
            if not filter_file_path.is_file():
                log_and_print("error", "Multi mode requires filter.txt file")
                sys.exit(1)
            
            filter_lines = read_file(filter_file_path).splitlines()
            field_names = read_field_names(field_file_path)
            
            # Filter out empty lines and comments
            active_filters = [line.strip() for line in filter_lines if line.strip() and not line.strip().startswith('#')]
            
            if not active_filters:
                log_and_print("error", "No valid filter conditions found in filter.txt")
                sys.exit(1)
            
            log_and_print("multi", f"Loaded {len(active_filters)} filter condition(s)")
            if field_names:
                log_and_print("multi", f"Field substitution enabled for: {', '.join(field_names)}")
            else:
                log_and_print("multi", "Using automatic field extraction from filter conditions")
            
            # Calculate total recipients for summary (in live mode)
            total_original_recipients = 0
            if not args.dry_run:
                for filter_condition in active_filters:
                    recipients_for_filter = apply_single_filter_logic(filter_condition, INVENTORY_PATH)
                    total_original_recipients += len(recipients_for_filter)
            else:
                # In dry-run, calculate for informational purposes
                for filter_condition in active_filters:
                    recipients_for_filter = apply_single_filter_logic(filter_condition, INVENTORY_PATH)
                    total_original_recipients += len(recipients_for_filter)
            
            # Show Multi Mode summary
            log_and_print("confirmation", f"=== MULTI MODE EMAIL SUMMARY ===")
            log_and_print("confirmation", f"From: {from_address}")
            log_and_print("confirmation", f"Subject Template: {subject_template}")
            log_and_print("confirmation", f"Individual Emails: {len(active_filters)}")
            
            if args.dry_run:
                log_and_print("confirmation", f"Mode: DRY-RUN (DRAFT emails to approvers)")
                log_and_print("confirmation", f"Approvers: {len(approver_emails)}")
                log_and_print("confirmation", f"Each approver will receive {len(active_filters)} draft emails")
                log_and_print("confirmation", f"Original campaign would target: {total_original_recipients} total recipients")
            else:
                total_live_recipients = total_original_recipients + (len(cc_emails) * len(active_filters)) + (len(bcc_emails) * len(active_filters))
                log_and_print("confirmation", f"Mode: LIVE")
                log_and_print("confirmation", f"Total TO Recipients: {total_original_recipients}")
                log_and_print("confirmation", f"CC per email: {len(cc_emails)} (total: {len(cc_emails) * len(active_filters)})")
                log_and_print("confirmation", f"BCC per email: {len(bcc_emails)} (total: {len(bcc_emails) * len(active_filters)})")
                log_and_print("confirmation", f"Grand Total: {total_live_recipients} emails")
            
            log_and_print("confirmation", f"Delay between emails: {args.delay}s")
            
            # Confirmation prompt unless --force
            if not args.force:
                if not prompt_for_confirmation():
                    log_and_print("info", "Multi mode email sending aborted by user.")
                    sys.exit(0)
            
            # Send Multi Mode emails
            ...
            log_and_print("info", "=== SINGLE MODE PROCESSING ===")
            ...
            if args.dry_run:
                ...
                if to_file_path.is_file():
                    ...
                    print()  # Add space before disclaimer
                    print(f"{BOLD}{RED}{'=' * 80}{RESET}")
                    print(f"{BOLD}{RED}                            ⚠️  IMPORTANT DISCLAIMER ⚠️{RESET}")
                    print(f"{BOLD}{RED}{'=' * 80}{RESET}")
                    print()
                    print(f"{BOLD}{YELLOW}⚠️  DISCLAIMER: Existing to.txt found - dry-run will NOT overwrite it{RESET}")
                    print(f"{BOLD}{BLUE}💡 To see fresh filter results, delete to.txt and run dry-run again{RESET}")
                    print(f"{BOLD}📋 Current to.txt contains {len(read_recipients(to_file_path))} recipients (preserving existing list){RESET}")
                    print()
                    print(f"{BOLD}{RED}{'=' * 80}{RESET}")
                    print()
                ...
                # Calculate original TO recipients - FIXED VERSION
                if to_file_path.is_file():
                    ...
                    # Apply deduplication to get accurate count - THIS IS THE FIX
                    if filtered_recipients:
                        # Use the same deduplication logic as write_recipients_to_file
                        seen = set()
                        unique_filtered = []
                        for email in filtered_recipients:
                            email_lower = email.lower()
                            if email_lower not in seen:
                                seen.add(email_lower)
                                unique_filtered.append(email)
                        original_recipients = unique_filtered
                    else:
                        original_recipients = []
                ...
                # Priority 2: Use filter logic if to.txt doesn't exist
                    ...
                    if recipients:
                        ...
                        actual_recipients = read_recipients(to_file_path)
                        recipients = actual_recipients
                        log_and_print("file", f"Created to.txt with {len(recipients)} unique recipients (was {original_count} before deduplication)")
            ...
            # Show Single Mode summary with CC/BCC info
            log_and_print("confirmation", f"=== SINGLE MODE EMAIL SUMMARY ===")
            log_and_print("confirmation", f"From: {from_address}")
            log_and_print("confirmation", f"Subject: {subject_template}")
            if args.dry_run:
                total_original = original_recipients_count + original_cc_count + original_bcc_count
                log_and_print("confirmation", f"Mode: DRY-RUN (DRAFT emails to approvers)")
            ...
            if not args.force:
                if not prompt_for_confirmation():
                    log_and_print("info", "Single mode email sending aborted by user.")
                    sys.exit(0)
            
            # Send emails in batches (Single Mode)
            send_email_batch(
                final_recipients, 
                subject_template, 
                body_template, 
        ...
        log_and_print("success", f"NotifyBot execution completed successfully ({operation_mode.upper()} MODE)")
