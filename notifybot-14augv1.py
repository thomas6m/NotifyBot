...
def combine_body_and_signature(body_html: str, signature_html: str) -> str:
    """
    Combine body HTML with signature HTML with minimal spacing.
    """
    if not signature_html:
        return body_html
    
    # Add minimal signature separator - just a single line break
    signature_separator = "\n<br>\n"  # Reduced from "\n<br><br>\n"
...
def sanitize_log_content(content: str, max_length: int = 100) -> str:
    """
    Sanitize content for CSV logging by removing/replacing problematic characters.
    """
    if not content:
        return content
...
def extract_field_values_from_matched_rows(filter_line: str, field_names: List[str], inventory_path: Path, base_folder: Path, mode: str = "single") -> Dict[str, str]:
    ...
    FIXED: Improved dynamic table generation logic with better CSV logging.
    """
    ...
    try:
        ...
        if has_dynamic_table_fields:
            ...
            # FIXED: Debug output to verify table generation - SANITIZED for CSV logging
            for field_name in DYNAMIC_FIELDS:
                if field_name in field_values and field_values[field_name]:
                    # FIXED: Sanitize the preview content for CSV logging
                    sanitized_preview = sanitize_log_content(field_values[field_name], 50)
                    log_and_print("info", f"Field '{field_name}' preview: {sanitized_preview}")
...
def get_recipients_for_multi_mode(base_folder: Path, dry_run: bool, test_mode: bool = False, custom_test_path: str = None) -> Tuple[List[Dict], List[str], List[str], int, int, int]:
    ...
    for i, filter_line in enumerate(filters, 1):
        ...
        if field_names:
            ...
            # Validate and report field extraction results - SANITIZED for CSV logging
            extracted_info = []
            empty_fields = []
            ...
            for field_name in field_names:
                ...
                if field_value:
                    ...
                    # FIXED: Sanitize display value for CSV logging
                    if value_count <= 3:
                        display_value = sanitize_log_content(field_value, 30)
                    else:
                        first_three = ','.join(field_value.split(',')[:3])
                        display_value = f"{sanitize_log_content(first_three, 20)}...+{value_count-3} more"
                    extracted_info.append(f"{field_name}=[{display_value}] ({value_count} unique)")
                else:
                    empty_fields.append(field_name)
            
            if extracted_info:
                # FIXED: Join the extracted info in a CSV-safe way
                extracted_summary = sanitize_log_content(', '.join(extracted_info), 200)
                log_and_print("info", f"Filter {i} successfully extracted: {extracted_summary}")
