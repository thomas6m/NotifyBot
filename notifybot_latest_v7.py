...
def send_via_sendmail(recipients: List[str], subject: str, body_html: str, 
    ...
    if dry_run:
        ...
        <div style="background-color: #fff3cd; border: 1px solid #ffeaa7; padding: 15px; margin: 20px 0; border-radius: 5px;">
            ...
            <p style="color: #856404; margin: 5px 0;"><strong>Original TO Recipients:</strong> {original_recipients_count}</p>
            <p style="color: #856404; margin: 5px 0;"><strong>Original CC Recipients:</strong> {original_cc_count}</p>
            <p style="color: #856404; margin: 5px 0;"><strong>Original BCC Recipients:</strong> {original_bcc_count}</p>
            <p style="color: #856404; margin: 5px 0;"><strong>Total Original Recipients:</strong> {original_recipients_count + original_cc_count + original_bcc_count}</p>
            <p style="color: #856404; margin: 5px 0;"><strong>This email is being sent to approvers for review.</strong></p>
            <p style="color: #856404; margin: 5px 0;">When approved, it will be sent to all intended recipients.</p>
        </div>
...
def send_email_batch(recipients: List[str], subject: str, body_html: str, 
    ...
    """Send emails in batches with CC/BCC sent only once with first batch."""
    ...
    if total_recipients == 0 and (cc_recipients or bcc_recipients):
        ...
        for i in range(0, total_recipients, batch_size):
            ...
            # CC/BCC only with FIRST batch to avoid duplicates
            current_cc = cc_recipients if batch_num == 1 else []
            current_bcc = bcc_recipients if batch_num == 1 else []
            ...
            if dry_run:
                ...
                if batch_num == 1 and (current_cc or current_bcc):
                    log_and_print("info", f"CC/BCC included with first batch only")
            
            # Send current batch with appropriate CC/BCC
    ...
    if dry_run:
        ...
        if successful_batches > 0:
            total_sent = original_recipients_count + (original_cc_count if successful_batches > 0 else 0) + (original_bcc_count if successful_batches > 0 else 0)
