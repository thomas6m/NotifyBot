...
recipient management through CSV-based filtering.
...
file attachments, CC/BCC recipients, and extensive logging.
...
- Dynamic table generation support
- Example: Department-specific notifications

FIELD VALIDATION SYSTEM:
------------------------

Priority-based validation ensures data integrity:

1. FILTER VALIDATION (filter.txt):
   - Always validated against /notifybot/inventory/inventory.csv
   - If local field-inventory.csv exists, also validated against it
   - All filter field names must exist in inventory headers
...
2. FIELD VALIDATION (field.txt):  
   ...
   - Dynamic table fields require table-columns.txt

3. STRICT DYNAMIC TABLE VALIDATION:
   - If any dynamic table field requested: table-columns.txt MUST exist
   - table-columns.txt MUST be non-empty
   - All columns MUST exist in chosen inventory file
...
OPERATORS:
  ...
  =~    Regex match: name=~".*Manager.*"
  !~    Regex not match: email!~".*(test|demo).*"
  *,?,[] Wildcard match: status=active*

LOGIC:
  AND: department="sales",region="north"    # Same line, comma-separated
  OR:  department="sales"                   # Different lines
...
DYNAMIC TABLES:
  ...
  {simple_table_rows}  # Simple table rows
  {csv_table_rows}     # Pipe-separated rows
...
TEXT FILES (.txt):
  ...
  - Email lists: semicolon-separated, one per line
  - Comments: Lines starting with # are ignored
  - Empty lines: Ignored

HTML FILES:
  - Valid HTML5 structure recommended
  - Inline CSS preferred for email compatibility
  - Image references: use relative paths to images/ folder
  - Template placeholders: {fieldname} syntax

CSV FILES:
  - UTF-8 encoding with BOM support
  - Headers required in first row
  - Email field: semicolon-separated for multiple emails
...
SUPPORTED:
  ...
  - Total size limit: 15MB per campaign
  - Automatic MIME type detection
  - Safe filename sanitization

LIMITATIONS:
  - No individual file size limit
  - Files over 10MB may cause delivery issues
...
FEATURES:
  ...
  - External URL warning (not embedded)

USAGE:
  1. Place images in images/ folder
  2. Reference in HTML: <img src="logo.png" alt="Logo">
  3. System automatically embeds and creates cid: references
...
BENEFITS:
  ...
  - Enables partial recovery from failures
...
FEATURES:
  ...
  - Shows original recipient counts
  - Preserves all formatting and attachments
  - Generates recipient backup files

WORKFLOW:
  1. Run with --dry-run flag
  2. Approvers receive test email
  3. Review content, formatting, attachments
  4. Run without --dry-run for live sending

LOGGING SYSTEM:
--------------

FORMAT: CSV-based structured logging
LOCATION: /notifybot/logs/notifybot.log  
LEVELS: INFO, WARNING, ERROR, SUCCESS

LOG ENTRIES INCLUDE:
  - Timestamp (milliseconds)
  - Username of executor
  - Emoji-coded message levels
...
ERROR PREVENTION:
  ...
  - Automatic field availability reporting
  - Syntax help for filter conditions

COMMAND LINE INTERFACE:
----------------------

REQUIRED ARGUMENTS:
  --base-folder FOLDER    Campaign folder name in /notifybot/basefolder/

OPTIONAL ARGUMENTS:
  --mode {single,multi}   Override mode.txt setting
  --dry-run               Send drafts to approvers only
...
USAGE EXAMPLES:
  ...
  # Live single-mode campaign
  notifybot.py --base-folder announcement --mode single --force
...
EMAIL SECURITY:
  ...
  - Recipient address syntax checking
  - Sendmail injection protection
  - Proper header encoding

FILE SECURITY:
  - Path traversal prevention
  - Base folder restriction enforcement
  - Safe filename sanitization
  - Size limit enforcement

DATA PROTECTION:
  - No external network access for images
  - Local file system access only
  - Comprehensive audit logging
  - Recipient data backup
...
COMMON ERROR SCENARIOS:
  - Missing required files → Clear file list provided
  - Invalid email addresses → Skipped with warning
  - Filter syntax errors → Syntax help displayed  
  - Network failures → Retry logic implemented
  - Large attachments → Size guidance provided
...
RECIPIENT MANAGEMENT:
  ...
  - Recipient backup creation
  - Multi-mode recipient tracking
  - Comprehensive recipient reports

DEBUGGING SUPPORT:
  - Verbose logging options
  - Template substitution tracking
  - Filter match reporting
...
Dependencies:
    ...
    - Standard library: pathlib, csv, re, subprocess, etc.
...
def get_inventory_path(test_mode: bool = False) -> Path:
    """Get inventory path based on test mode."""
    if test_mode:
        return NOTIFYBOT_ROOT / "inventory" / "test-inventory.csv"
    else:
        return NOTIFYBOT_ROOT / "inventory" / "inventory.csv"

def validate_fields_with_priority(base_folder: Path, mode: str = "single", test_mode: bool = False) -> Tuple[bool, List[str]]:
    """
    Validate fields in filter.txt and field.txt against inventory headers.
    Strict dynamic table check: table-columns.txt must exist and be valid if any dynamic table field is requested.
    """
    errors = []
    inventory_path = get_inventory_path(test_mode)
    ...
    # Load local inventory headers if present
    ...
    if has_local_field_inventory:
        ...
        log_and_print("info", "No local field-inventory.csv found")

    # Validate filter.txt fields against global inventory
                ...
                for condition in conditions:
                    ...
                    for op in ['=~', '!~', '!=', '=']:
                        if op in condition:
                            ...
                            break
                    if field_name and field_name not in global_available_fields:
    ...
    if field_file.is_file():
        try:
            ...
            if normalized_fields & DYNAMIC_FIELDS:
                ...
                # Validate table columns against chosen inventory
                available_fields = local_available_fields if has_local_field_inventory else global_available_fields
                invalid_cols = [col for col in table_fields if col not in available_fields]
                if invalid_cols:
                    errors.append(f"Fields in table-columns.txt not found in inventory: {', '.join(invalid_cols)}")
                    return False, errors
...
def check_required_files(base: Path, required: List[str], dry_run: bool = True, mode: str = "single", test_mode: bool = False) -> None:
...
def get_recipients_for_single_mode(base_folder: Path, dry_run: bool, test_mode: bool = False) -> Tuple[List[str], List[str], List[str], int, int, int]:
...
def extract_field_values_from_matched_rows(filter_line: str, field_names: List[str], inventory_path: Path, base_folder: Path, mode: str = "single") -> Dict[str, str]:
    ...
    This function already accepts inventory_path as parameter, so no changes needed to function body.
    """
    ...
    try:
        ...
        for field in field_names:
            if field in headers and field not in {"table_headers", "dynamic_table", "table_rows", "styled_table_rows", "simple_table_rows", "csv_table_rows"}:
                values = set()
        ...
        # Strict dynamic table check
        has_dynamic_table_fields = any(f in {"dynamic_table", "table_rows", "styled_table_rows", "simple_table_rows", "csv_table_rows", "table_headers"} for f in field_names)
...
def get_recipients_for_multi_mode(base_folder: Path, dry_run: bool, test_mode: bool = False) -> Tuple[List[Dict], List[str], List[str], int, int, int]:
...
def get_inventory_fields_for_help(test_mode: bool = False) -> str:
    """
    ...
    """
    inventory_path = get_inventory_path(test_mode)
    inventory_name = "test-inventory.csv" if test_mode else "inventory.csv"
    
    try:
        if not inventory_path.exists():
            return f"  [Inventory file not found at {inventory_path}]"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="NotifyBot - Advanced Email Campaign Manager with Single/Multi Mode Support",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=f"""
OVERVIEW:
=========
NotifyBot is an advanced email campaign manager supporting two operational modes:
...
Mode Selection Priority:
  1. --mode CLI argument (highest priority)
  2. mode.txt file in base folder
  3. Default: single mode

SINGLE MODE:
  - Sends ONE email to multiple recipients
  - Recipients from: to.txt, filter.txt, additional_to.txt, cc.txt, bcc.txt
  - Supports batching for large recipient lists
  - Template substitution with field.txt (optional)

MULTI MODE:
  - Sends MULTIPLE personalized emails based on filter conditions
  - Each filter condition creates a separate email campaign
  - Advanced template substitution with dynamic content
...
SINGLE MODE Requirements:
  ...
  Recipient sources (at least ONE required):
    - to.txt             # Direct recipient list (semicolon-separated)
  ...
  Optional files:
    - field.txt          # Field names for template substitution
    - mode.txt           # Force mode selection (single/multi)
  
  Optional folders:
    - attachment/        # Files to attach to emails
    - images/           # Images to embed in HTML content
...
MULTI MODE Requirements:
  Required files:
    ...
    - from.txt          # Sender email address
    - approver.txt      # Approver emails for dry-run mode
    - filter.txt        # Filter conditions (one per email)
  
  Optional files:
    - field.txt         # Field names for template substitution
    - additional_to.txt # Additional recipients for each email
    - cc.txt           # CC recipients for each email
    - bcc.txt          # BCC recipients for each email
    - mode.txt         # Force mode selection
    - table-columns.txt # Required if dynamic table fields used
  
  Optional folders:
    - attachment/       # Files to attach to each email
    - images/          # Images to embed in HTML content

INVENTORY SYSTEM & FIELD VALIDATION:
====================================

Global Inventory:
  Location: /notifybot/inventory/inventory.csv
  Purpose: Central repository of all available data fields
  Usage: Required for filter.txt validation and field lookups

Local Field Inventory (Optional):
  Location: <base-folder>/field-inventory.csv
  Purpose: Override field validation for specific campaigns
  Usage: If present, field.txt validation uses this instead of global

Priority-based Field Validation:
  1. filter.txt fields → ALWAYS validated against global inventory
  2. field.txt fields → Validation priority:
     a) Local field-inventory.csv (if exists)
     b) Global inventory.csv (fallback)
  3. If local field-inventory.csv exists:
     - filter.txt fields ALSO validated against local inventory
...
Supported Operators:
  ...
  *?[]  Wildcard patterns (*, ?, character classes)

Logic Operations:
  AND: Use comma separation within same line
  OR:  Use separate lines for different conditions

Examples:
  department="sales"                          # Exact match
  region!="europe"                           # Not equal
  name=~".*Manager.*"                        # Regex: contains "Manager"
  email!~".*(test|demo).*"                   # Regex: exclude test/demo emails
  status=active*                             # Wildcard: starts with "active"
  department="sales",region="north"          # AND: sales AND north
  department="sales"                         # OR condition (line 1)
  department="marketing"                     # OR condition (line 2)
  # This is a comment                       # Comments supported

Advanced Filter Examples:
  # Active sales team in specific regions
  status="active",department="sales",region=~"(north|south)"
  
  # Exclude test accounts and inactive users
  email!~"test",status!="inactive"
  
  # Manager-level positions with wildcards
  title=*manager*,status="active"

TEMPLATE SUBSTITUTION:
======================

Basic Fields:
  Use {{field_name}} in subject.txt and body.html
  Values extracted from matching inventory rows
  Comma-separated values automatically formatted

Dynamic Table Fields (Multi Mode):
  {{table_headers}}     # Formatted HTML table headers
  {{table_rows}}        # Standard HTML table rows
  {{styled_table_rows}} # Striped table rows with styling
  {{simple_table_rows}} # Simple table rows (minimal styling)
  {{csv_table_rows}}    # Pipe-separated table data
  {{dynamic_table}}     # Alias for table_rows

Table Column Configuration:
  File: table-columns.txt
  Content: One column name per line
  Requirement: MANDATORY if any table field is used
  Validation: All columns must exist in inventory

DRY-RUN MODE:
=============

Purpose: Test email campaigns safely before live sending

Features:
  - Sends DRAFT emails to approvers only
  - Preserves original recipient data for reference
  - Adds campaign information banner to email body
  - Subject prefixed with "DRAFT -"
  - Shows original recipient counts in email content

Safety Features:
  - Never sends to actual recipients in dry-run
  - Creates to.txt files for future reference
  - Validates all files and data integrity
  - Provides comprehensive campaign preview
...
Features:
  ...
  - Size limit validation (15MB default)
  - Safe filename sanitization

Size Limits:
  - Total attachment size: 15MB maximum
  - Individual file limits: No specific limit
  - Validation occurs during pre-flight checks

Supported Formats:
  - Documents: PDF, DOC, DOCX, TXT, etc.
  - Images: JPG, PNG, GIF, etc.
  - Archives: ZIP, RAR, etc.
  - Any MIME type supported by email standards

IMAGE EMBEDDING:
================

Automatic Processing:
  - Images in images/ folder automatically embedded
  - HTML img tags converted to cid: references
  - Reduces email size and improves deliverability

Supported Formats:
  - JPG, JPEG, PNG, GIF, BMP
  - Automatic MIME type detection
  - Inline embedding with proper headers

Usage in HTML:
  <img src="logo.png" alt="Company Logo">
  # Automatically becomes: <img src="cid:image_0_logo_png" alt="Company Logo">

BATCHING SYSTEM:
================

Purpose:
  - Prevent mail server overload
  - Comply with sending limits
  - Provide controlled delivery pace

Configuration:
  - --batch-size: Recipients per batch (default: 500)
  - --delay: Seconds between batches (default: 5.0)
  - Automatic batch calculation and reporting

Behavior:
  - CC/BCC included in every batch
...
Single Mode Recipients:
  - to.txt: Primary recipients
  - filter.txt: Dynamic recipient selection
  - additional_to.txt: Extra recipients to merge
  - Automatic deduplication
  - Priority-based source selection

Multi Mode Recipients:
  - Each filter creates separate recipient list
  - CC/BCC applied to every email
  - additional_to.txt merged with each filter
  - Comprehensive recipient tracking and backup

Validation Features:
  - Email format validation
  - Sendmail compatibility checks
...
Log Format: CSV with timestamps and user tracking
...
Log Levels:
  ...
  🔧 MODE: Mode selection
  ✍️  SIGNATURE: Signature processing
...
Directory Structure:
  /notifybot/
  ├── basefolder/
  │   └── <your-campaign>/     # Your campaign folder
  │       ├── subject.txt
  │       ├── body.html
  │       ├── from.txt
  │       ├── approver.txt
  ...
  │       ├── bcc.txt (optional)
  │       ├── mode.txt (optional)
  │       ├── table-columns.txt (optional)
  │       ├── field-inventory.csv (optional)
  │       ├── attachment/ (optional)
  │       ├── images/ (optional)
  │       └── recipients/ (generated)
  ├── inventory/
  │   └── inventory.csv         # Global inventory
  ├── logs/
...
Single Mode with Custom Settings:
  python notifybot.py --base-folder newsletter \\
    --mode single --batch-size 300 --delay 10 --force

Multi Mode Campaign:
  python notifybot.py --base-folder segmented-promo \\
    --mode multi --delay 15

Large Campaign with Batching:
  python notifybot.py --base-folder quarterly-report \\
    --batch-size 100 --delay 30 --dry-run

Force Live Send (Skip Confirmation):
  python notifybot.py --base-folder urgent-update \\
    --force --batch-size 1000

TROUBLESHOOTING:
================

Common Issues:
  - "Missing required files": Check file requirements above
  - "Invalid email format": Verify email addresses in source files
  - "Field validation failed": Ensure field names exist in inventory
  - "Attachment size limit exceeded": Reduce attachment total size
  - "No recipients found": Check filter conditions and inventory data
...
Pre-flight Checks:
  ...
  - Inventory compatibility validation

Debug Tips:
  - Use --dry-run first
  - Check /notifybot/logs/notifybot.log
  - Verify inventory.csv headers
  - Test with small recipient lists
  - Validate filter conditions manually
        """
    )
    ...
    parser.add_argument("--test", action="store_true", help="Use test-inventory.csv instead of inventory.csv for testing")
    
    parser.add_argument("--mode", 
                       choices=['single', 'multi'], 
                       metavar="MODE",
                       help="Override mode: 'single' (one email to many) or 'multi' (many personalized emails)")
    
    parser.add_argument("--dry-run", 
                       action="store_true", 
                       help="SAFE MODE: Send draft emails to approvers only (no actual recipients)")
    ...
    setup_logging()
    if args.test:
        log_and_print("mode", "TEST MODE: Using test-inventory.csv")
    ...
    try:
        ...
        check_required_files(base_folder, required_files, args.dry_run, mode, args.test)
        ...
        if mode == "single":
            (final_recipients, final_cc_recipients, final_bcc_recipients, 
             original_recipients_count, original_cc_count, original_bcc_count) = get_recipients_for_single_mode(base_folder, args.dry_run, args.test)
            ...
            (email_configs, final_cc_recipients, final_bcc_recipients, 
             total_original_recipients_count, original_cc_count, original_bcc_count) = get_recipients_for_multi_mode(base_folder, args.dry_run, args.test)
